...
<div class="panel-heading">决策区域</div>
  ...
  <form id="taskForm" target="uploadFileIFrame" class="form" action="file/upload" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group">
      ...
      <div class="radio">
        <label>
          <input type="radio" name="decision" id="decision1" value="ok" checked><span>通过概念方案</span>
        </label>
      </div>
      <div class="radio">
        <label>
          <input type="radio" name="decision" id="decision2" value="nok"><span>驳回概念方案</span>
        </label>
      </div>
...
var completeTask = function() {
  ...
  gotoPage("/xmod/azdProject/completeTask", 
	      {taskId:"<%=request.getAttribute("taskId") %>", comment:comment, decision:decision, decisionInfo:decisionInfo, attachmentIds:attachmentIds});
};
</script>