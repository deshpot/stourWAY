...
<div class="panel-heading">决策区域</div>
  ...
  <form id="taskForm" target="uploadFileIFrame" class="form" action="file/upload" method="post" enctype="multipart/form-data">
	  <div class="form-group">
	    <label for="buyer">概念方案负责人：</label>
	    <select id="conceptSolutionManager" class="form-control">
	    </select>
	    <label for="buyer">正式方案负责人：</label>
	    <select id="detailedSolutionManager" class="form-control">
	    </select>
      <label for="buyer">扩初设计负责人：</label>
      <select id="conceptDesignManager" class="form-control">
      </select>
	    <label for="buyer">施工图负责人：</label>
      <select id="detailedDesignManager" class="form-control">
      </select>
	  </div>
    <div class="form-group">
...
$.ajax({
  ...
  success : function(data) {
    ...
    $("#conceptSolutionManager").html(userOptionHtml);
    $("#detailedSolutionManager").html(userOptionHtml);
    $("#conceptDesignManager").html(userOptionHtml);
    $("#detailedDesignManager").html(userOptionHtml);
  },
...
var completeTask = function() {
  var conceptSolutionManager = $("#conceptSolutionManager").val();
  var detailedSolutionManager = $("#detailedSolutionManager").val();
  var conceptDesignManager = $("#conceptDesignManager").val();
  var detailedDesignManager = $("#detailedDesignManager").val();
  
  var comment=$("#comment").val();
  gotoPage("/xmod/azdProject/completeTask", 
	      {taskId:"<%=request.getAttribute("taskId") %>", 
	  			conceptSolutionManager:conceptSolutionManager,
				detailedSolutionManager:detailedSolutionManager,
				conceptDesignManager:conceptDesignManager,
				detailedDesignManager:detailedDesignManager,
				comment:comment, decisionInfo:"制定项目计划", attachmentIds:attachmentIds});
};
</script>