...
<div class="panel-heading">决策区域</div>
  ...
  <form id="taskForm" target="uploadFileIFrame" class="form" action="file/upload" method="post" enctype="multipart/form-data">
    <div class="form-group">
      <button type="button" onclick="addTask()">增加任务</button>
      <div id="assignTask"></div> 
      <div id="taskTemplate" style="display:none;">
        <label for="assignee">人员：</label>
        <select id="assignee">
        </select>
        <label for="taskDescription">任务：</label>
        <textarea id="taskDescription" rows="3" placeholder="内务内容描述"></textarea>
        <button type="button" onclick="removeTask(this)">删除任务</button>
      </div>
    </div>
    <div class="form-group">
...
var addTask = function() {
  $("#assignTask").append($("#taskTemplate").clone().attr("id","task").show());
};
var removeTask = function(btnObj) {
  $(btnObj).parent().remove();
};
$.ajax({
  type : "post",
...
var completeTask = function() {
  ...
  $.each($("#assignTask").find("div#task"), function(index, obj) {
    tasks += ($(obj).find("#assignee").val()+" -=STSD=-"+$(obj).find("#taskDescription").val()+" -=STSD=-no comment")+" -=-=STSD=-=-";
  });
  
  gotoPage("/xmod/azdProject/completeTask", 
	      {taskId:"<%=request.getAttribute("taskId") %>", comment:comment, tasks:tasks, decisionInfo:"正式方案计划安排", attachmentIds:attachmentIds});
};
</script>